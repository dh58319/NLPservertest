...
if __name__ == '__main__':
    ...
    print("created table")
    normal_words = load_dic('data/english_word.txt')
    print("# words in dictionary:", len(normal_words))
    valid_nearest = []
    valid_nearest_mat = None
    eliminated = 0
    checked_words = set()
    total_lines = count_lines('data/cc.en.300.vec') - 1
    with open('data/cc.en.300.vec', 'r', encoding='utf-8', errors='ignore') as w2v_file:
        _ = w2v_file.readline()
        ...
        for n, line in enumerate(w2v_file):
            ...
            word = words[0]
            word = unicodedata.normalize('NFC', word)
            if not is_english(word) or word in checked_words:
                eliminated += 1
            else:
